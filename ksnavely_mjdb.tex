...
The construction effort is taking place in a cleanroom evironment, including the electroforming
of the most radiopure copper ever produced \cite{phillips_mj}.
...
a drastic departure from their humble beginnings \cite{ginige_web}. Users can run software without
...
The parts tracking database web portal is a data-centric workflow driven application -- it is designed
to allow scientists to record important data and present a useful interface for data inspection. The software takes an 
...
The Backbone View, in turn, will refresh the UI on model change. 
The view objects contain logic and functions
for interaction with the Document Object Model (DOM), the objects which represent page content. 
...
the view is rerendered. User interaction with DOM elements trigger events which are caught by the view and the appropriate
...
for their guidance while developing the PTDB software.
A special thanks to M. Green, M. Busch, and R. Varner for providing great
