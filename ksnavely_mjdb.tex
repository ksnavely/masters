...
cosmogenic activation in materials and parts used by the \textsc{Majorana} collaboration. The
\textsc{Majorana} \textsc{Demonstrator} is an R\&D proof of concept leading towards a tonne-scale Germanium
...
\IEEEPARstart{T}{he} \textsc{Majorana} \textsc{Demonstrator} is an ultra-low background Physics
experiment searching for the neutrinoless double beta decay (\znbb) process.
Detecting \znbb\ would reveal whether the neutrinos are their own antiparticles,
...
WIMPs are the leading candidates for the composition of the majority of dark matter
in the universe, which comprises REF 26\% of the mass-energy density in the universe.
...
One part of \textsc{Majorana}'s radiopurity campaign is a comprehensive parts tracking effort. Cosmic
...
radiation due to activated detector components. \textsc{Majorana} has developed a CouchDB based database
...
\textsc{Majorana} to push the limits of experimental cleanliness and materials radiopurity. One source of background that
...
can be tracked over time an estimate can be provided for cosmogenic activation rates. This will allow \textsc{Majorana}
...
Part history is also referenced during construction to ensure a component is suitable in terms of radiopurity or cleanliness.
...
purpose. The material of the part must be known, for instance in the case of ultra-pure electroformed copper versus commercial
copper. A detailed history aids quality assurance during the assembly of experiment hardware.
...
a non-expert user to interface with data stored in \textsc{Majorana}'s CouchDB. Any web browser becomes a tool which can
...
to easily add process types directly from the user interface. Some processes require official \textsc{Majorana} procedure to be
followed and entry of the \textsc{Majorana} procedure number is required.

\textsc{Majorana} manufactures its own detector components deep underground in its clean machine shop, and tracking the natural
...
although an assembly may be a component of another assembly.
...
frontend and calculation results are pushed to the CouchDBdatabase.
...
Accessing the JSON documents in a CouchDBdatabase is done via HTTP/REST. The REST model prescribes a client-server
...
In the CouchDBJSON record, lists of relatives or linked histories are simply arrays of database ids. To provide a rich
...
returned from a CouchDBview is used to instantiate a SearchResults collection which implements various
sorting features.
