...
of their exposure to cosmic radiation. Access to part data also provides
...
\IEEEPARstart{D}{etecting} neutrinoless double-beta decay (\znbb) would reveal whether neutrinos are their own antiparticle,
...
around the \znbb\ signal at 2038 keV.
The \textsc{Demonstrator}'s extreme radiopurity requirements also make it competitive in the search for dark
matter candidate particles known as Weakly Interacting Massive Particles (WIMPs).
WIMPs are a leading candidate for the composition of dark matter,
which composes 26.8\% \cite{darkmatter_bertone} \cite{plank_results} of the mass-energy density in the universe.

Achieving this low background goal requires a multifaceted strategy including shielding, assembly in a clean environment,
careful materials choice, and a comprehensive parts tracking effort.
To shield the experiment from radiation, it is located deep underground
...
The construction effort is taking place in a class 1000 clean evironment, including the electroforming
of the most radiopure copper ever produced. REF Assay result?

Parts tracking pays dividends not only in logistics but can also help realize the background goal.
...
for data entry and a Python based exposure calculator that converts storage and transportation history 
into time and elevation profiles which can be used to limit
material activity.
...
is noted and material type inherited. Throught the part's lifetime it receives history entries which range from storage
...
to red-flag parts which may be highly radioactive, or aid in troubleshooting if a source of background
...
components can be identified through metal stamping or affixing labels. Groups of small identical parts are often bagged together
and a range of serial numbers listed on the bag.
...
and others. A history entry references one or more locations which are stored in document in the database. Each location has address meta-data
...
platform for application development. Users can execute software without
the need for code management and compiling, and technologies proven on the web such as HyperText Markup Language (HTML)
and JavaScript can provide a graphical user interface and interactivity. 

The PTDB web application takes an 
Object-Oriented approach and it should be noted that JavaScript does not support class-based inheritance as
in C++ (or similar languages), but rather implements behavior reuse through prototype-based inheritance. If a desired
...
In this paper we will use the term class as a taxonomal reference and may speak of
...
which is also used by the web. This allows applications which run in the browser to mesh naturally with CouchDB.
Unlink Structured Query Language (SQL) databases, CouchDB stores information in JavaScript Object Notation (JSON)
documents. 
...
as if it were a single CouchDB instance. This eliminates a hardware bottleneck when executing
resource intensive tasks with no added development work.
...
may deal with HTML and Cascading Style Sheets (CSS), which are entirely seperate from the data's area of concern.
...
method which updates the HTML on the page. The view objects also contain logic and functions
for interaction with the Document Object Model (DOM). Though a view has a reference to the model object it represents,
...
to singleton Collections in the PTDB, and this is how views communicate via messages
...
This decouples models from other models or views and the
...
convienient methods for loop constructs and common functional programming tasks.
...
server mode. Using the framework simply involves writing callback functions for the appropriate request URI.
...
unfinished linking operations due to browser crash, closing, etc...), or querying
...
key component of \textsc{Majorana}'s radiopurity campaign and will help the collaboration achieve
...
\begin{itemize}
  \item \textbf{JQuery} -- http://jquery.com/
   ...
   DOM interaction and AJAX requests. It also has a nice UI
   toolkit which is used in the web application.
  \item \textbf{Underscore.js} -- http://underscorejs.org/

   Underscore is a great library for loop constructs and
   functional programming style. It is required by Backbone.js.
...
\begin{itemize}
  \item \textbf{Twisted} -- http://twistedmatrix.com/trac/
   ...
   JSON exposure data via HTTP. At it's simplist, it turns a URI
   request into function execution.
\end{itemize}

% use section* for acknowledgement
\section*{Acknowledgment}
I would like to thank R. Henning and J. Wilkerson for all of their
help and direction in my time at The University of North Carolina. Also
thanks to M. Green, M. Busch, and R. Varner for providing great
feedback during software development and putting the software to use.
Everyone in \textsc{Majorana} and at SURF is owed thanks for being
excellent scientists and coworkers. 
